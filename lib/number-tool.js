...
(function (global) {
        ...
        '<div class="inner">' +
          ...
          '<div class="fields">' +
            ...
            '<fieldset><legend>keys</legend><p class="keys-hint">' +
              ...
              '<br>trackpad scroll or drag face/chart/row to nudge · <kbd>⇧</kbd> fast · pinch coarse · arrows on focused field' +
              '<br>touch: drag chart, face, or any value row · stack scrolls · steppers hold to accelerate' +
            '</p></fieldset>' +
          '</div>' +
        '</div>' +
      '</div>';
  ...
  function mountGestures() {
    ...
    function onPointerDown(e) {
      ...
      /* Chart / face scrub the primary instrument value */
      if (!locked && (face || stage)) locked = primaryInput();
    ...
    function onPointerMove(e) {
      ...
      if (absY >= absX) {
        ...
        while (Math.abs(drag.accumX) >= thresh) {
          ...
          /* Horizontal on a locked row still nudges that row (not axis-x remap) */
