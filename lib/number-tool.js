...
(function (global) {
  ...
  function deepVegaConfig() {
    ...
    var face = (cs.getPropertyValue('--face') || '#fafaf8').trim();
    var hair = (cs.getPropertyValue('--hair') || border).trim();
    ...
    return {
      ...
      range: { ramp: [face, hair, border, accent], category: [mute, border, fg, accent] },
      point: { color: accent },
      rule: { color: accent },
      mark: { color: fg },
  ...
  /** Cache-bust key when theme / chrome / token colors change. */
  function deepLookKey() {
    var cs = getComputedStyle(document.documentElement);
    var root = document.documentElement;
    return [
      root.getAttribute('data-theme'),
      root.getAttribute('data-ui'),
      root.getAttribute('data-font'),
      root.getAttribute('data-faces'),
      (cs.getPropertyValue('--accent') || '').trim(),
      (cs.getPropertyValue('--mute') || '').trim(),
      (cs.getPropertyValue('--rule') || '').trim(),
      (cs.getPropertyValue('--face') || '').trim()
    ].join('|');
  }
