...
(function (global) {
        ...
        '<div class="inner">' +
          ...
          '<div class="fields">' +
            ...
            '<fieldset><legend>keys</legend><p class="keys-hint">' +
              ...
              '<br>trackpad scroll or drag face/row to nudge · <kbd>⇧</kbd> fast · pinch coarse · arrows on focused field' +
              '<br>touch: drag face or value row · stack scrolls · steppers hold to accelerate' +
            '</p></fieldset>' +
          '</div>' +
        '</div>' +
      '</div>';
  ...
  /**
   * Trackpad + touch gestures: wheel/pinch + pointer drag scrub on face/rows.
   * Focused/hovered number row wins; else data-primary.
   */
  function mountGestures() {
    ...
    function isInteractiveTarget(t) {
      ...
      return !!(
        t.closest('button, a, select, textarea, .spin, .seg, .presets, #settings, .suite-nav, .suite-chrome, .icon-btn') ||
        (t.matches && t.matches('input'))
      );
    }
    ...
    function onPointerDown(e) {
      ...
      /* Typing: don't steal vertical pans that start on the focused field */
      if (row && row.matches(':focus-within') && t.closest('input')) return;
    ...
    function onPointerMove(e) {
      ...
      if (absY >= absX) {
        while (Math.abs(drag.accumY) >= thresh) {
          ...
          nudge(drag.locked || resolveTarget('y'), ydir, { fast: e.shiftKey });
        }
        drag.accumX = 0;
      } else {
        while (Math.abs(drag.accumX) >= thresh) {
          ...
          nudge(drag.locked || resolveTarget('x'), xdir, { fast: e.shiftKey });
        }
        drag.accumY = 0;
      }
    }
